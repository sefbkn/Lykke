    ...
    public partial class MsgTx
    {
        ...
        private void DecodeWitnessValueSigning(BinaryReader reader)
        {
            ...
            if (count > MaxTxInPerMessage)
                throw new EncodingException($"transaction count {count} too high to fit into message");
        ...
        private TxIn ReadTxInPrefix(BinaryReader reader, TxSerializeType serializationType)
        {
            if (serializationType == TxSerializeType.OnlyWitness)
                throw new System.Exception("tried to read a prefix input for a witness only tx");
