...
{
    public class Blake256 : HashAlgorithm
    {
        ...
        public Blake256()
        {
            HashSizeValue = 256; // Hash size in bits
            Initialize();
        }
        ...
        {
            return pb[iOffset + 3] | ((uint) pb[iOffset + 2] << 8) |
        ...
        private void Compress(byte[] pbBlock, int iOffset)
        {
            ...
            for (var i = 0; i < 8; ++i) m_h[i] ^= m_v[i];
            for (var i = 0; i < 8; ++i) m_h[i] ^= m_v[i + 8];

            for (var i = 0; i < 4; ++i) m_h[i] ^= m_s[i];
            for (var i = 0; i < 4; ++i) m_h[i + 4] ^= m_s[i];
        }
        ...
        {
            var pbMsgLen = new byte[8];
            var uLen = m_t + ((ulong) m_nBufLen << 3);
